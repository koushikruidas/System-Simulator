...
@Component
public class DatabaseScenarioConfigurer implements NodeScenarioConfigurer {

	@Override
	public ApiNodeType supportedType() {
		return ApiNodeType.DATABASE;
	}

	@Override
	public void configure(ScenarioBuilder builder, NodeCommand nodeCommand) {
		int capacity = requiredPositive(nodeCommand.capacity(), "Database node " + nodeCommand.nodeId() + " must define a positive capacity");
		int queueLimit = defaultInteger(nodeCommand.queueLimit(), capacity);
		builder.addDatabase(nodeCommand.nodeId(), capacity, queueLimit, defaultLong(nodeCommand.latency(), 0L));
	}
